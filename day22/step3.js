// 1. JS가 head에 있으므로 생성되기전에 호출이므로 오류발생.
console.log(document.querySelector('img')); 

// 2. JS가 head에 있으나 DOMcontentLoaded 이벤트를 사용했으므로 오류 발생
document.addEventListener('DOMContentLoaded',function(){
    console.log(document.querySelector('img'));

    // [p.315]
    // 1. class가 'rect'안 요소를 여러개 호출
    ...
    for(let index=0; index<rects.length; index++){
        ...
        // rects[index].setAttribute('src',src);   // 속성에 값을 지정
    ...
    for(let i=0; i<divs.length; i++){
        ...
        const val=i*10;     // 0*10=0, 1*10=10, 2*10=20
        divs[i].style.height='10px';
        divs[i].style.backgroundColor=`rgba(${val},${val},${val})`;
    }

    // [p.320] 요소 만들기
    const header=document.createElement('h1'); // h1 요소생성 [1. 생성]
        console.log(header);
